...
export class spielbrettGrafik
{
    ...
    zeichneSpielBrett(fensterBreite: number, fensterHoehe: number): void
    {
        ...
        this.ctx.beginPath();
        this.ctx.moveTo(this.abstand, this.abstand);
        this.ctx.lineTo(7 * this.abstand, this.abstand);
        this.ctx.stroke();

        this.ctx.beginPath();
        this.ctx.moveTo(this.abstand, this.abstand);
        this.ctx.lineTo(this.abstand, 7 * this.abstand);
        this.ctx.lineTo(7 * this.abstand, 7 * this.abstand);
        this.ctx.lineTo(7 * this.abstand, this.abstand);
        this.ctx.lineTo(this.abstand, this.abstand);
        this.ctx.stroke();

        this.ctx.beginPath();
        this.ctx.moveTo(2 * this.abstand, 2 * this.abstand);
        this.ctx.lineTo(6 * this.abstand, 2 * this.abstand);
        this.ctx.lineTo(6 * this.abstand, 6 * this.abstand);
        this.ctx.lineTo(2 * this.abstand, 6 * this.abstand);
        this.ctx.lineTo(2 * this.abstand, 2 * this.abstand);
        this.ctx.stroke();

        this.ctx.beginPath();
        this.ctx.moveTo(3 * this.abstand, 3 * this.abstand);
        this.ctx.lineTo(5 * this.abstand, 3 * this.abstand);
        this.ctx.lineTo(5 * this.abstand, 5 * this.abstand);
        this.ctx.lineTo(3 * this.abstand, 5 * this.abstand);
        this.ctx.lineTo(3 * this.abstand, 3 * this.abstand);
        this.ctx.stroke();

        this.ctx.beginPath();
        this.ctx.moveTo(4 * this.abstand, this.abstand);
        this.ctx.lineTo(4 * this.abstand, 3 * this.abstand);
        this.ctx.stroke();

        this.ctx.beginPath();
        this.ctx.moveTo(4 * this.abstand, 5 * this.abstand);
        this.ctx.lineTo(4 * this.abstand, 7 * this.abstand);
        this.ctx.stroke();

        this.ctx.beginPath();
        this.ctx.moveTo(this.abstand, 4 * this.abstand);
        this.ctx.lineTo(3 * this.abstand, 4 * this.abstand);
        this.ctx.stroke();

        this.ctx.beginPath();
        this.ctx.moveTo(5 * this.abstand, 4 * this.abstand);
        this.ctx.lineTo(7 * this.abstand, 4 * this.abstand);
        this.ctx.stroke();

        // Zeichne Rand fuer Aussensteine
        const abstandHalbe = Math.round(this.abstand / 2 );

        this.ctx.fillStyle = hintergrundfarbe;
        this.ctx.strokeStyle = 'black';
        this.ctx.fillRect(3 * this.abstand - abstandHalbe, (8 + this.muehleComponent.faktor) * this.abstand - abstandHalbe,
                          this.abstand , this.abstand);
        this.ctx.strokeRect(3 * this.abstand - abstandHalbe, (8 + this.muehleComponent.faktor) * this.abstand - abstandHalbe,
                          this.abstand , this.abstand);

        this.ctx.fillRect(5 * this.abstand - abstandHalbe, (8 + this.muehleComponent.faktor) * this.abstand - abstandHalbe,
        this.abstand , this.abstand);
        this.ctx.strokeRect(5 * this.abstand - abstandHalbe, (8 + this.muehleComponent.faktor) * this.abstand - abstandHalbe,
                          this.abstand , this.abstand);
        ...
        if (this.muehleComponent.getAktuelleStellung() != null)
        {
           ...
           for (let bitNr = 1; bitNr < 25; bitNr++)
           {
                ...
                {
                    this.zeichneSpielstein(Math.round(this.abstand * (this.muehleComponent.grafikSpielsteinPositionen[bitNr][0])),
                                           Math.round(this.abstand * (this.muehleComponent.grafikSpielsteinPositionen[bitNr][1])),
                                           'white', 'black' );
                }
           }
           ...
           {
                this.zeichneSpielstein(3 * this.abstand , (8 + this.muehleComponent.faktor) * this.abstand , 'white', 'black');

                this.ctx.font = '12px Arial';
                this.ctx.fillStyle = 'black';
                this.ctx.fillText(this.muehleComponent.getAktuelleStellung().getAnzahlSteineAussen()[0].toString(),
                             3 * this.abstand - 3, (8 + this.muehleComponent.faktor) * this.abstand + 5);
           }
           if (this.muehleComponent.getAktuelleStellung().getAnzahlSteineAussen()[1] > 0)
           {
                this.zeichneSpielstein(5 * this.abstand, (8 + this.muehleComponent.faktor) * this.abstand, 'black', 'white');

                this.ctx.font = '12px Arial';
                this.ctx.fillStyle = 'white';
                this.ctx.fillText(this.muehleComponent.getAktuelleStellung().getAnzahlSteineAussen()[1].toString(),
                            5 * this.abstand - 3, (8 + this.muehleComponent.faktor) * this.abstand + 5);
           }
    ...
    zeichneSpielstein(x: number, y: number, fuellFarbe: string, randFarbe: string): void{
        ...
        this.ctx.beginPath();
        this.ctx.arc(x, y, Math.round(this.abstand * (1 - this.muehleComponent.faktor) / 2), 0, 2 * Math.PI);
        this.ctx.fill();
        this.ctx.lineWidth = 1;
    ...
    zeichneEinKreuz(posStein: number, farbe: string): void{
        if (typeof posStein !== 'undefined' && posStein !== 0)
        {
            ...
            this.ctx.beginPath();
            this.ctx.moveTo(Math.round(this.abstand * (this.muehleComponent.grafikSpielsteinPositionen[posStein][0] -
                               this.muehleComponent.faktor) + this.abstand / 4),
                            Math.round(this.abstand * (this.muehleComponent.grafikSpielsteinPositionen[posStein][1] -
                               this.muehleComponent.faktor) + this.abstand / 4));

            this.ctx.lineTo(Math.round(this.abstand * (this.muehleComponent.grafikSpielsteinPositionen[posStein][0] -
                               this.muehleComponent.faktor) + this.abstand * 3 / 7),
                            Math.round(this.abstand * (this.muehleComponent.grafikSpielsteinPositionen[posStein][1] -
                               this.muehleComponent.faktor) + this.abstand * 3 / 7));
            this.ctx.stroke();
